...
module.exports = {
  'should be able to set and retrieve local state' : function() {
    var g = new Gossiper();
    g.setLocalState('hi', 'hello');
    assert.equal('hello', g.getLocalState('hi'));
  },
  'should be able to get a list of keys for a peer' : function() {
    var g = new Gossiper();
  ...
  'should be able to get the value of a key for a peer' : function() {
    var g = new Gossiper();
    g.peers['p1'] = new PeerState();
    g.peers['p1'].attrs['keyz'] = ['hi', 1];
    assert.equal('hi', g.peerValue('p1','keyz'));
  },
  'should be able to get a list of peers' : function() {
    var g = new Gossiper();
    g.peers['p1'] = new PeerState();
