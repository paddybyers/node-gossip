...
var Gossiper = exports.Gossiper = function(port, seeds) {
  ...
  for (i in opt_args) {
    ...
    if (arg.constructor === String) {
      this.ip_to_bind = this.address = arg;
    };
...
Gossiper.prototype.start = function(callback) {
  ...
  this.server = net.createServer(function (net_stream) {
    var jstream = new jsonstream.Stream(net_stream);
    jstream.on('msg', function(msg) { self.handleMessage(net_stream, jstream, msg) });
  });
  ...
  if(this.ip_to_bind) {
    ...
    // listen by default on the same address that
    // the server identifies itself as
    self.server.listen(self.port, os.hostname(), function() {
      var address = self.server.address().address;
...
Gossiper.prototype.gossip = function() {
  ...
  if(live_peer && !this.seeds[live_peer] && this.livePeers().length < this.seeds.length) {
    if(Math.random() < (this.seeds / this.allPeers.length)) {
...
Gossiper.prototype.gossipToPeer = function(peer) {
  ...
  // don't gossip to myself
  if (peer === this.peer_name) {
    return;
  }
  ...
  if (!self.conns[peer]) {
    ...
    gosipee.on('connect', function(net_stream) {
      var jstream = new jsonstream.Stream(gosipee);
      jstream.on('msg', function(msg) { self.handleMessage(gosipee, jstream, msg) });
      jstream.send(self.requestMessage());
      self.conns[peer] = jstream;
    });
    gosipee.on('error', function(e) {
      //console.log(self.peer_name + " received " + util.inspect(exception));
    });
    gosipee.on('end', function() {
      delete self.conns[peer];
    });
  } else {
    self.conns[peer].send(self.requestMessage());
  }
}
...
Gossiper.prototype.handleMessage = function(net_stream, jstream, msg) {
  switch(msg.type) {
    case Gossiper.REQUEST:
      if (msg.username === this.username && msg.password === this.password) {
        jstream.send(this.firstResponseMessage(msg.digest));
      } else {
    ...
    case Gossiper.FIRST_RESPONSE:
      ...
      jstream.send(this.secondResponseMessage(msg.request_digest));
      break;
...
Gossiper.prototype.handleNewPeers = function(new_peers) {
  ...
  for(var i in new_peers) {
    ...
    if(!(peer_name in this.peers) && peer_name != this.my_state.name) {
